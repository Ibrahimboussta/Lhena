        ...
        <div class="w-full max-w-6xl grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div class="md:col-span-2 bg-white rounded-2xl shadow p-3 sm:p-4 space-y-4">
                ...
                <div class="flex items-center gap-2">
                    <span class="text-yellow-400 text-lg">{{ str_repeat('★', $avgRating) }}<span class="text-gray-300">{{ str_repeat('☆', 5 - $avgRating) }}</span></span>
                ...
                <div x-data="{
                    ...
                    <!-- Fullscreen Modal -->
                    <div x-show="fullscreenOpen" x-cloak x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" x-transition:leave="transition ease-in duration-200" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0" class="fixed inset-0 w-full h-full z-[99999] bg-black flex items-center justify-center" @click.self="closeFullscreen">
                        ...
                        <div class="w-full h-full flex items-center justify-center p-4 md:p-12">
                            <img :src="'{{ asset('storage') }}/' + photos[fullscreenIndex]" class="max-h-full max-w-full object-contain rounded-lg shadow-2xl" @click.stop>
                        </div>

                        <!-- Fullscreen Counter -->
                        <div class="absolute bottom-4 md:bottom-8 left-1/2 -translate-x-1/2 text-white bg-black/70 px-4 py-2 rounded-full text-sm z-[100001] border border-white/20" x-text="(fullscreenIndex + 1) + ' / ' + photos.length"></div>
                    </div>
            ...
            <div class="md:col-span-1">
                <div class="sticky top-16 bg-white rounded-2xl shadow p-6 space-y-4">
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <a href="https://www.google.com/maps?q={{ urlencode($property->address) }}" target="_blank" class="text-gray-700 hover:text-emerald-600">
                            {{ $property->address }}
                        </a>
                    </div>
                    ...
                    <div class="space-y-2">
                        ...
                        <button onclick="openAgentModal()" class="inline-flex items-center justify-center w-full bg-emerald-600 text-white px-6 py-2.5 text-sm font-medium rounded-lg hover:bg-emerald-700 transition-all duration-200">
                            <span>📞</span>
                    ...
                    <!-- Login Modal -->
                    <div id="loginModal" class="fixed inset-0 bg-black bg-opacity-50 items-center justify-center z-[99999]" :class="{ 'hidden': !modalOpen, 'flex': modalOpen }" x-data="{ modalOpen: false }" x-show="modalOpen" x-cloak>
                        <div class="bg-white p-6 rounded-lg max-w-md w-full">
                            ...
                            <div class="flex justify-end space-x-2">
                                <a href="{{ route('register') }}" class="bg-emerald-600 text-white px-4 py-2 rounded-lg hover:bg-emerald-700 transition">Se
                                    connecter</a>
                                <button @click="modalOpen = false" class="px-4 py-2 rounded-lg border hover:bg-gray-100 transition">Annuler</button>
                            </div>
                        </div>
                    </div>
    ...
    <div class="mt-10 border-t border-gray-200 pt-8 max-w-6xl mx-auto">
        <h3 class="flex items-center justify-center text-3xl font-bold text-gray-900 mb-8 gap-3">
            <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6 text-emerald-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 8h2a2 2 0 012 2v9a2 2 0 01-2 2h-6l-4 4v-4H7a2 2 0 01-2-2v-1" />
            </svg>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            @auth
            <form x-data="{ loading: false }" @submit="loading = true" action="{{ route('reviews.store') }}" method="POST" class="space-y-4 bg-white shadow-md border border-gray-200 rounded-xl p-6 h-fit">
                @csrf
                    ...
                    <div class="flex items-center gap-2">
                        <template x-for="i in 5" :key="i">
                            <label class="cursor-pointer">
                                ...
                                <svg xmlns="http://www.w3.org/2000/svg" class="w-7 h-7 transition-colors duration-200" :class="i <= rating ? 'text-yellow-400' : 'text-gray-300'" viewBox="0 0 20 20" fill="currentColor">
                ...
                <div class="flex justify-end">
                    <button type="submit" class="inline-flex items-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-medium px-5 py-2 rounded-lg transition disabled:opacity-50 disabled:cursor-not-allowed" :disabled="loading">
            ...
            <div class="bg-white border border-gray-200 rounded-xl p-6 shadow-sm text-center h-fit">
                ...
                <div class="flex justify-center gap-3">
                    <a href="{{ route('login') }}" class="px-4 py-2 rounded-md bg-red-600 text-white text-sm font-medium hover:bg-red-700 transition">
                        Se connecter
                    </a>
                    <a href="{{ route('register') }}" class="px-4 py-2 rounded-md bg-emerald-600 text-white text-sm font-medium hover:bg-emerald-700 transition">
                        S’inscrire
                    </a>
                </div>
            </div>
        ...
        <div class="flex justify-center">
            <div class="bg-white border border-gray-200 rounded-xl p-6 text-center shadow-sm">
                <svg xmlns="http://www.w3.org/2000/svg" class="mx-auto w-12 h-12 text-gray-400 mb-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9.75 17L4.5 12.75l5.25-4.25M14.25 7l5.25 4.25-5.25 4.25" />
                </svg>
            ...
            <a href="{{ route('proprites.details', $property->slug) }}" class="group">
                <div class="rounded-2xl border border-gray-200 bg-white overflow-hidden shadow-sm hover:shadow-lg transition duration-300 flex flex-col">
                    ...
                    <div class="relative w-full h-64 overflow-hidden">
                        <img class="w-full h-full object-cover group-hover:scale-105 transition duration-500" src="{{ asset('storage/' . json_decode($property->photos)[0]) }}" alt="" loading="lazy">
                    ...
                    <div class="flex-1 flex flex-col p-4">
                        ...
                        <div class="flex items-center mt-4 text-gray-600 text-sm gap-x-6">
                            ...
                            <div class="flex items-center space-x-1">
                                <img class="w-4 h-4 opacity-70" src="{{ asset('images/beds.svg') }}" alt="" loading="lazy">
                                <span>{{ $property->bedrooms }}</span>
                            </div>

                            <div class="flex items-center space-x-1">
                                <img class="w-4 h-4 opacity-70" src="{{ asset('images/dosh.svg') }}" alt="" loading="lazy">
                                <span>{{ $property->bathrooms }}</span>
                            </div>

                            <div class="flex items-center space-x-1">
                                <img class="w-4 h-4 opacity-70" src="{{ asset('images/space.svg') }}" alt="" loading="lazy">
